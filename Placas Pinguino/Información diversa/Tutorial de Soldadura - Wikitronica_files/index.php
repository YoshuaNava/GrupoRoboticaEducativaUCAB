...
wikEd is a full-featured advanced text editor for regular to advanced users on Wikipedia and MediaWikis.
...
wikEd.InitGlobalConfigs = function() {
	...
	wikEd.InitText = function() {
		wikEd.InitObject(wikEd.config.text, {
			...
			'wikEdDiff title':             'Enable wikEdDiff (improved diff view)',
	...
	if (typeof(wikEd.config.tableMode) == 'undefined') { wikEd.config.tableMode = true; }
	...
	if (typeof(wikEd.config.externalApiUrl) == 'undefined') { wikEd.config.externalApiUrl = '//commons.wikimedia.org/w/api.php'; }
	...
	if (typeof(wikEd.config.externalApiDomains) == 'undefined') { wikEd.config.externalApiDomains = 'wikipedia.org,wiktionary.org,wikibooks.org,wikinews.org,wikiquote.org,wikisource.org,wikiversity.org,wikivoyage.org,wikidata.org,mediawiki.org,wikimedia.org,wikimediafoundation.org'; }
	...
	if (typeof(wikEd.config.nbspToChar) == 'undefined') { wikEd.config.nbspToChar = false; }
...
wikEd.Startup = function() {
	...
	wikEd.programVersion = /@version\s+(.*?)[ \t]*\n/.exec(meta)[1];
	wikEd.programDate = /@date\s+(.*?)[ \t]*\n/.exec(meta)[1];
...
wikEd.TurnOn = function(scrollToEditFocus) {
	...
	if ( (typeof(mw) == 'object') && (typeof(mw.user) == 'object') && (typeof(mw.user.options) == 'object') ) {
		if ( (mw.user.options.get('usebetatoolbar') == 1) && (mw.user.options.get('usecodeeditor') == 1) ) {
			codeEditor = 1;
		}
	}
	...
	if (wikEd.textarea == null) {
		...
		if ( (textarea != null) && (textarea.type != 'hidden') ) {
		...
		switch (event.type) {
			...
			case 'keyup':
				switch (event.keyCode) {
					...
					case 33: // page up
					case  8: // backspace
						wikEd.AntiHighlightBleeding(new Object(), null, 'left');
						break
					case 39: // right
					case 40: // down
					case 34: // page down
						wikEd.AntiHighlightBleeding(new Object(), null, 'right');
						break
				}
				break;
	...
	for (var i = 0; i < spans.length; i ++) {
		...
		if ( (id != null) && (id.indexOf('wikEdWikiLink') == 0) ) {
			if (wikEd.wikiLinks.hasOwnProperty(id) == true) {
				...
				if (wikEd.externalLinkInfo.hasOwnProperty(externalLink) == true) {
...
wikEd.HideShowHandler = function(event) {
	...
	if ( (event.type == 'mouseover') || (event.type == 'mouseout') || (event.type == 'click') ) {
		...
		if  ( (hideTarget.tagName == 'TD') && (/^wikEdTable\w+?$/.test(hideTarget.className) == true) ) {
			hideCell = hideTarget;
			hideButton = hideCell.getElementsByTagName('button')[0];
		}
...
wikEd.HighlightBuildTree = function(tag, tagClass, tagFrom, tagLength, parseObj, tagProperties) {
	...
	if ( (pushNode != null) && (tagProperties != null) ) {
...
wikEd.HighlightAddHtml = function(parseObj, obj) {
	...
	while (i < parseObj.tree.length) {
		...
		var attrib = '';
		if ( (node.attrib != null) && (node.attrib != '') ) {
			var htmlTag = '';
			switch (tag) {
				case 'table':
					htmlTag = 'table';
		...
		var paired = null;
		if ( (node.paired != null) && (node.paired > 0) ) {
...
wikEd.UpdateFrame = function(html) {
	...
	if (html != null) {
		...
		// convert nbsp to character entities so they do not get converted to blanks
...
wikEd.GetGlobalsReceiver = function(event) {
	...
	if (event.data != '') {
		...
		if (globals != null) {
			...
			if ( (globalObj.hookNumber != null) && (wikEd.gotGlobalsHook[globalObj.hookNumber] != null) ) {
